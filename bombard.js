...
class Bombard {
    constructor(canvas, game, playerNumber, name, lives = 1){
        this.canvas = canvas;
    ...
    handleBrickWallCollision() {       // !!!!    Handle other player collision???
        this.game.bombards.forEach(bombard=>{
            if(bombard.identifier == this.identifier) {
    ...
    handleNoteBombCollision() {        //Not implemented
        this.game.bombards.forEach(bombard=>{
            if(bombard.identifier == this.identifier) {
    ...
    didCollide(somethingWithXYandSize) {
    ...
    handleArrivingToGoal() {               //!!! collides with square to the left of goal
